...
test.describe("OTP and access approval regressions", () => {
  test.skip(!shouldRun, "Set E2E_RUN=true to execute e2e regressions.");

  test("OTP-required login path works end-to-end", async ({ request }) => {
    const seed = Date.now().toString(36);
    const userEmail = `otp.flow.${seed}@example.com`;
    const password = "Passw0rd!Secure";

    const signupUser = await request.post(`${apiBase}/api/user/`, {
      data: { firstname: "Otp", lastname: "Flow", email: userEmail, password }
    });
    expect([200, 409]).toContain(signupUser.status());

    const loginBefore2faRes = await request.post(`${apiBase}/api/user/login`, {
      data: { email: userEmail, password }
    });
    expect(loginBefore2faRes.ok()).toBeTruthy();
    const loginBefore2fa = parseApi<{ accessToken?: string }>(await loginBefore2faRes.json());
    const initialAccessToken = String(loginBefore2fa.accessToken || "");
    expect(initialAccessToken.length).toBeGreaterThan(20);

    const setup2faRes = await request.post(`${apiBase}/api/user/generate-2fa`, {
      headers: { Authorization: `Bearer ${initialAccessToken}` },
      data: {}
    });
    expect(setup2faRes.ok()).toBeTruthy();
    const setup2fa = parseApi<{ secret?: string }>(await setup2faRes.json());
    const totpSecret = String(setup2fa.secret || "");
    expect(totpSecret.length).toBeGreaterThan(10);

    const challengeRes = await request.post(`${apiBase}/api/user/login`, {
      data: { email: userEmail, password }
    });
    expect(challengeRes.status()).toBe(401);
    const challengeBody = await challengeRes.json();
    const challengeData = parseApi<{ otpRequired?: boolean; requiresOtp?: boolean }>(challengeBody);
    expect(challengeData.otpRequired || challengeData.requiresOtp).toBeTruthy();

    const otp = generateTotp(totpSecret);
    const otpLoginRes = await request.post(`${apiBase}/api/user/login`, {
      data: { email: userEmail, password, otp }
    });
    expect(otpLoginRes.ok()).toBeTruthy();
    const otpLogin = parseApi<{ accessToken?: string }>(await otpLoginRes.json());
    expect(String(otpLogin.accessToken || "").length).toBeGreaterThan(20);
  });

  test("approved request becomes decryptable", async ({ request }) => {
    const seed = Date.now().toString(36);
    const adminEmail = `admin.approve.${seed}@example.com`;
    const userEmail = `user.approve.${seed}@example.com`;
    const password = "Passw0rd!Secure";

    const signupAdmin = await request.post(`${apiBase}/api/admin/`, {
      data: { firstname: "Admin", lastname: "Approve", email: adminEmail, password }
    });
    expect([200, 409]).toContain(signupAdmin.status());

    const signupUser = await request.post(`${apiBase}/api/user/`, {
      data: { firstname: "User", lastname: "Approve", email: userEmail, password }
    });
    expect([200, 409]).toContain(signupUser.status());
    ...
    const preApproveDecryptRes = await request.post(`${apiBase}/api/user/decrypt`, {
      headers: { Authorization: `Bearer ${userAccess}` },
      data: {
        fileId,
        secretKeyBase64,
    ...
    expect(preApproveDecryptRes.status()).toBe(403);

    const requestAccessRes = await request.post(`${apiBase}/api/user/request-access`, {
      headers: { Authorization: `Bearer ${userAccess}` },
      data: { fileId, description: "Need to decrypt after approval" }
    });
    ...
    const pending = parseApi<{ requests?: Array<{ _id: string; email: string; fileId?: { _id?: string } | string }> }>(
      await pendingRes.json()
    );
    const targetRequest = (pending.requests || []).find((entry) => {
      const pendingFileId = typeof entry.fileId === "string" ? entry.fileId : entry.fileId?._id;
      return entry.email === userEmail && String(pendingFileId || "") === fileId;
    });
